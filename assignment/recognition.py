...
def resize_image(image, size):
    ...
    resized_image = cv2.resize(image, size, interpolation=cv2.INTER_AREA).astype(np.uint8)
...
def binarize(image, **binarization_kwargs):
    ...
    binarized_image = cv2.adaptiveThreshold(image, 
                                            255, 
                                            cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
                                            cv2.THRESH_BINARY_INV, 
                                            binarization_kwargs.get('block_size'), 
                                            binarization_kwargs.get('c_constant'))
...
def get_sudoku_cells(frontalized_image, crop_factor=0.63, binarization_kwargs={}):
    """
    Args:
    ...
    for i in range(NUM_CELLS):
        for j in range(NUM_CELLS):
            y_top = i * CELL_SIZE[0]
            y_bottom = (i + 1) * CELL_SIZE[0]
            x_left = j * CELL_SIZE[0]
            x_right = (j + 1) * CELL_SIZE[0]

            sudoku_cell = binarized_image[y_top:y_bottom, x_left:x_right]
...
def is_empty(sudoku_cell, **kwargs):
    ...
    white_pixel_ratio = np.sum(sudoku_cell == 255) / sudoku_cell.size
    
    cell_is_empty = white_pixel_ratio < kwargs.get('threshold')
...
def get_digit_correlations(sudoku_cell, templates_dict):
    ...
    if is_empty(sudoku_cell, threshold=0.05):
        return correlations

    for digit, templates in templates_dict.items():
        max_correlation = 0
        # calculate the correlation score between the sudoku_cell and a digit
        # and take the maximum correlation across all templates
        for template in templates:
            # normalized cross-correlation
            correlation_map = match_template(sudoku_cell, template, pad_input=True)
            
            # get maximum correlation score from the map
            current_max = np.max(correlation_map)
            max_correlation = max(max_correlation, current_max)

        # save best correlation between Sudoku cell and current digit
